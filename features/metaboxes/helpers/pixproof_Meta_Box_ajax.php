...
class pixproof_Meta_Box_ajax {
	...
	public function oembed_handler() {
		...
		// verify our nonce
		if ( ! ( isset( $_REQUEST['pixproof_ajax_nonce'], $_REQUEST['oembed_url'] ) && wp_verify_nonce( $_REQUEST['pixproof_ajax_nonce'], 'ajax_nonce' ) ) )
			die();
	...
	public static function get_oembed( $url, $object_id, $args = array() ) {
		...
		if ( isset( $args['object_type'] ) && $args['object_type'] != 'post' ) {
			...
			add_filter( 'get_post_metadata', array( 'pixproof_Meta_Box_ajax', 'hijack_oembed_cache_get' ), 10, 3 );
			// Sets ombed cache in our object's meta (vs postmeta)
			add_filter( 'update_post_metadata', array( 'pixproof_Meta_Box_ajax', 'hijack_oembed_cache_set' ), 10, 4 );
		...
		if ( $check_embed && $check_embed != $fallback )
			return '<div class="embed_status">'. $check_embed .'<p><a href="#" class="pixproof_remove_file_button" rel="'. $args['field_id'] .'">'. __( 'Remove Embed', 'cmb' ) .'</a></p></div>';
	...
	public static function hijack_oembed_cache_get( $check, $object_id, $meta_key ) {
		...
		$data = 'options-page' === self::$object_type
			? pixproof_Meta_Box::get_option( self::$object_id, self::$embed_args['cache_key'] )
	...
	public static function hijack_oembed_cache_set( $check, $object_id, $meta_key, $meta_value ) {
		...
		if ( 'options-page' === self::$object_type ) {
			// Set the option
			pixproof_Meta_Box::update_option( self::$object_id, self::$embed_args['cache_key'], $meta_value, array( 'type' => 'oembed' ) );
			// Save the option
			pixproof_Meta_Box::save_option( self::$object_id );
		} else {
