...
class pixproof_Meta_Box_types {
	 ...
	 * @var   pixproof_Meta_Box_types object
	 * @since 1.0.0
	 */
	...
	/**
	 * Holds pixproof_valid_img_types
	 * @var   array
	 * @since 1.0.0
	 */
	 ...
	 * @since  1.0.0
	 * @return pixproof_Meta_Box_types A single instance of this class.
	 */
	...
	private static function desc( $paragraph = false ) {
		...
		$desc = pixproof_Meta_Box::$field['desc'];
		return "\n<$tag class=\"pixproof_metabox_description\">$desc</$tag>\n";
	}
	...
	private static function repeat_text_field( $meta, $class = '', $type = 'text' ) {
		...
		$field = pixproof_Meta_Box::$field; self::$meta = $meta; self::$type = $type;
	...
	private static function text_input( $class = '', $val = '' ) {
		...
		$before = '';
		if ( pixproof_Meta_Box::$field['type'] == 'text_money' )
			$before = ! empty( pixproof_Meta_Box::$field['before'] ) ? ' ' : '$ ';
		return $before . '<input type="'. self::$type .'" class="'. $class .'" name="'. pixproof_Meta_Box::$field['id'] .'[]" id="'. pixproof_Meta_Box::$field['id'] .'_'. self::$iterator .'" value="'. self::esc( $val ) .'" data-id="'. pixproof_Meta_Box::$field['id'] .'" data-count="'. self::$iterator .'"/>';
	}
	...
	private static function repeat_table_open( $class = '' ) {
		echo self::desc(), '<table id="', pixproof_Meta_Box::$field['id'], '_repeat" class="cmb-repeat-table ', $class ,'"><tbody>';
	}
	...
	private static function repeat_table_close() {
		echo '</tbody></table><p class="add-row"><a data-selector="', pixproof_Meta_Box::$field['id'] ,'_repeat" class="add-row-button button" href="#">'. __( 'Add Row', 'cmb' ) .'</a></p>';
	}
	...
	public static function is_valid_img_ext( $file ) {
		...
		self::$valid = empty( self::$valid ) ? (array) apply_filters( 'pixproof_valid_img_types', array( 'jpg', 'jpeg', 'png', 'gif', 'ico', 'icon' ) ) : self::$valid;
	...
	public static function esc( $meta_value, $func = '' ) {
		$field = pixproof_Meta_Box::$field;

		// Check if the field has a registered escaping callback
		$cb = pixproof_Meta_Box::maybe_callback( $field, 'escape_cb' );
		if ( false === $cb ) {
		...
		$esc = apply_filters( 'pixproof_types_esc_'. $field['type'], null, $meta_value, $field );
		if ( null !== $esc ) {
			return $esc;
		}
	...
	public static function text_small( $field, $meta ) {
		if ( $field['repeatable'] ) {
			return self::repeat_text_field( $meta, 'pixproof_text_small' );
		}

		echo '<input class="pixproof_text_small" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', self::esc( $meta ), '" />', self::desc();
	}

	public static function text_medium( $field, $meta ) {
		if ( $field['repeatable'] ) {
			return self::repeat_text_field( $meta, 'pixproof_text_medium' );
		}
		echo '<input class="pixproof_text_medium" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', self::esc( $meta ), '" />', self::desc();
	}

	public static function text_email( $field, $meta ) {
		if ( $field['repeatable'] ) {
			return self::repeat_text_field( $meta, 'pixproof_text_email pixproof_text_medium', 'email' );
		}

		echo '<input class="pixproof_text_email pixproof_text_medium" type="email" name="', $field['id'], '" id="', $field['id'], '" value="', self::esc( $meta ), '" />', self::desc( true );
	}

	public static function text_url( $field, $meta ) {
		if ( $field['repeatable'] ) {
			return self::repeat_text_field( $meta, 'pixproof_text_url pixproof_text_medium' );
		}

		echo '<input class="pixproof_text_url pixproof_text_medium regular-text" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', self::esc( $meta, 'esc_url' ), '" />', self::desc( true );
	}

	public static function text_date( $field, $meta ) {
		echo '<input class="pixproof_text_small pixproof_datepicker" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', self::esc( $meta ), '" />', self::desc();
	}

	public static function text_date_timestamp( $field, $meta ) {
		echo '<input class="pixproof_text_small pixproof_datepicker" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', ! empty( $meta ) ? date( 'm\/d\/Y', $meta ) : $field['default'], '" />', self::desc();
	}

	public static function text_datetime_timestamp( $field, $meta, $object_id ) {

		// This will be used if there is a select_timezone set for this field
		$tz_offset = pixproof_Meta_Box::field_timezone_offset( $object_id );
		if ( ! empty( $tz_offset ) ) {
			$meta -= $tz_offset;
		}

		echo '<input class="pixproof_text_small pixproof_datepicker" type="text" name="', $field['id'], '[date]" id="', $field['id'], '_date" value="', ! empty( $meta ) ? date( 'm\/d\/Y', $meta ) : $field['default'], '" />';
		echo '<input class="pixproof_timepicker text_time" type="text" name="', $field['id'], '[time]" id="', $field['id'], '_time" value="', ! empty( $meta ) ? date( 'h:i A', $meta ) : $field['default'], '" />', self::desc();
	}
	...
	public static function text_datetime_timestamp_timezone( $field, $meta ) {
		...
		echo '<input class="pixproof_text_small pixproof_datepicker" type="text" name="', $field['id'], '[date]" id="', $field['id'], '_date" value="', ! empty( $meta ) ? date( 'm\/d\/Y', $meta ) : $field['default'], '" />';
		echo '<input class="pixproof_timepicker text_time" type="text" name="', $field['id'], '[time]" id="', $field['id'], '_time" value="', ! empty( $meta ) ? date( 'h:i A', $meta ) : $field['default'], '" />';
	...
	public static function text_time( $field, $meta ) {
		echo '<input class="pixproof_timepicker text_time" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', self::esc( $meta ), '" />', self::desc();
	}

	public static function select_timezone( $field, $meta ) {
		$meta = self::esc( $meta );
		if ( '' === $meta )
			$meta = pixproof_Meta_Box::timezone_string();
	...
	public static function text_money( $field, $meta ) {
		if ( $field['repeatable'] ) {
			return self::repeat_text_field( $meta, 'pixproof_text_money' );
		}

		echo ! empty( $field['before'] ) ? '' : '$', ' <input class="pixproof_text_money" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', self::esc( $meta ), '" />', self::desc();
	}
	...
	public static function colorpicker( $field, $meta ) {
		...
		elseif ( ! preg_match( '/^#' . $hex_color . '/i', $meta ) ) // Value doesn't match #123abc, so sanitize to just #.
			$meta = "#";
		echo '<input class="pixproof_colorpicker pixproof_text_small" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', $meta, '" />', self::desc();
	}
	...
	public static function textarea_code( $field, $meta ) {
		echo '<pre><textarea name="', $field['id'], '" id="', $field['id'], '" cols="60" rows="10" class="pixproof_textarea_code">', ! empty( $meta ) ? $meta : $field['default'], '</textarea></pre>', self::desc( true );
	}
	...
	public static function radio( $field, $meta ) {
		...
		foreach ( $field['options'] as $option_key => $option ) {
			...
			echo '<li class="pixproof_option"><input type="radio" name="', $field['id'], '" id="', $field['id'], $i,'" value="', $value, '" ', checked( $meta == $value ), ' /> <label for="', $field['id'], $i, '">', $label ,'</label></li>';
			$i++;
		}
	...
	public static function checkbox( $field, $meta ) {
		echo '<input class="pixproof_option" type="checkbox" name="', $field['id'], '" id="', $field['id'], '" ', checked( ! empty( $meta ) ), ' value="on"/> <label for="', $field['id'], '">', self::desc() ,'</label>';
	}
	...
	public static function multicheck( $field, $meta ) {
		...
		foreach ( $field['options'] as $value => $name ) {
			echo '<li><input class="pixproof_option" type="checkbox" name="', $field['id'], '[]" id="', $field['id'], $i, '" value="', $value, '" ', checked( is_array( $meta ) && in_array( $value, $meta ) ), '  /> <label for="', $field['id'], $i, '">', $name, '</label></li>';
			$i++;
		}
		echo '</ul>', self::desc();
	}
	...
	public static function title( $field, $meta, $object_id, $object_type ) {
		...
		echo '<'. $tag .' class="pixproof_metabox_title">', $field['name'], '</'. $tag .'>', self::desc( true );
	}
	...
	public static function taxonomy_radio( $field, $meta, $object_id ) {
		...
		foreach ( $terms as $term ) {
			...
			echo '<li class="pixproof_option"><input type="radio" name="', $field['id'], '" id="', $field['id'], $i,'" value="'. $term->slug . '" ', checked( $checked ), ' /> <label for="', $field['id'], $i, '">' . $term->name . '</label></li>';
			$i++;
		}
		echo '</ul>', self::desc( true );
	}
	...
	public static function taxonomy_multicheck( $field, $meta, $object_id ) {
		...
		$i = 1;
		foreach ( $terms as $term ) {
			echo '<li><input class="pixproof_option" type="checkbox" name="', $field['id'], '[]" id="', $field['id'], $i,'" value="'. $term->slug . '" ';
	...
	public static function file_list( $field, $meta, $object_id ) {
		...
		echo '<input class="pixproof_upload_button button pixproof_upload_list" type="button" value="'. __( 'Add or Upload File', 'cmb' ) .'" />', self::desc( true );
			...
			foreach ( $meta as $id => $fullurl ) {
				if ( self::is_valid_img_ext( $fullurl ) ) {
					...
					'<li class="img_status">',
						...
						'<p><a href="#" class="pixproof_remove_file_button">'. __( 'Remove Image', 'cmb' ) .'</a></p>
					...
					echo
					'<li>',
						__( 'File:', 'cmb' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $fullurl, '" target="_blank" rel="external">'. __( 'Download', 'cmb' ) .'</a> / <a href="#" class="pixproof_remove_file_button">'. __( 'Remove', 'cmb' ) .'</a>)
						<input type="hidden" id="filelist-', $id ,'" name="', $field['id'] ,'[', $id ,']" value="', $fullurl ,'" />
					</li>';
				}
			}
	...
	public static function file( $field, $meta, $object_id, $object_type ) {
		...
		echo '<input class="pixproof_upload_button button" type="button" value="'. __( 'Add or Upload File', 'cmb' ) .'" />';

		$_id_name = $field['id'] .'_id';

		$_id_meta = pixproof_Meta_Box::get_data( $_id_name );

		// If there is no ID saved yet, try to get it from the url
		if ( $meta && ! $_id_meta ) {
			$_id_meta = pixproof_Meta_Box::image_id_from_url( esc_url_raw( $meta ) );
		}

		echo '<input class="pixproof_upload_file_id" type="hidden" id="', $_id_name, '" name="', $_id_name, '" value="', $_id_meta, '" />',
		self::desc( true ),
		'<div id="', $field['id'], '_status" class="pixproof_media_status">';
			if ( ! empty( $meta ) ) {

				if ( self::is_valid_img_ext( $meta ) ) {
					echo '<div class="img_status">';
					echo '<img style="max-width: 350px; width: 100%; height: auto;" src="', $meta, '" alt="" />';
					echo '<p><a href="#" class="pixproof_remove_file_button" rel="', $field['id'], '">'. __( 'Remove Image', 'cmb' ) .'</a></p>';
					echo '</div>';
				} else {
					...
					echo __( 'File:', 'cmb' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $meta, '" target="_blank" rel="external">'. __( 'Download', 'cmb' ) .'</a> / <a href="#" class="pixproof_remove_file_button" rel="', $field['id'], '">'. __( 'Remove', 'cmb' ) .'</a>)';
				}
			}
		echo '</div>';
	}
	...
	public static function oembed( $field, $meta, $object_id, $object_type ) {
		echo '<input class="pixproof_oembed regular-text" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', self::esc( $meta ), '" data-objectid="', $object_id ,'" data-objecttype="', $object_type ,'" />', self::desc( true );
		echo '<p class="cmb-spinner spinner" style="display:none;"><img src="'. admin_url( '/images/wpspin_light.gif' ) .'" alt="spinner"/></p>';
		echo '<div id="', $field['id'], '_status" class="pixproof_media_status ui-helper-clearfix embed_wrap">';

			if ( $meta != '' )
				echo pixproof_Meta_Box_ajax::get_oembed( $meta, $object_id, array(
	...
	public static function text_small_repeat( $field, $meta ) {
		self::repeat_text_field( $meta, 'pixproof_text_small' );
	}
	public static function text_medium_repeat( $field, $meta ) {
		self::repeat_text_field( $meta, 'pixproof_text_medium' );
	}
	public static function text_email_repeat( $field, $meta ) {
		self::repeat_text_field( $meta, 'pixproof_text_email pixproof_text_medium', 'email' );
	}
	public static function text_url_repeat( $field, $meta ) {
		...
		self::repeat_text_field( $val, 'pixproof_text_url pixproof_text_medium' );
	}
	public static function text_money_repeat( $field, $meta ) {
		self::repeat_text_field( $meta, 'pixproof_text_money' );
	}


	/**
	 * Default fallback. Allows rendering fields via "pixproof_render_$name" hook
	 * @since  1.0.0
	...
	public function __call( $name, $arguments ) {
		...
		do_action( "pixproof_render_$name", $field, $meta, $object_id, $object_type );
	}
