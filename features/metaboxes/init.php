...
spl_autoload_register('pixproof_Meta_Box::autoload_helpers');

$meta_boxes = array();
$meta_boxes = apply_filters( 'pixproof_meta_boxes', $meta_boxes );
foreach ( $meta_boxes as $meta_box ) {
	$my_box = new pixproof_Meta_Box( $meta_box );
}

define( 'PIXPROOF_META_BOX_URL', pixproof_Meta_Box::get_meta_box_url() );

/**
 * Create meta boxes
 */
class pixproof_Meta_Box {
	...
	protected static $mb_defaults = array(
		...
		'pixproof_styles' => true, // Include cmb bundled stylesheet
		'fields'     => array(),
	);
	...
	function __construct( $meta_box ) {
		...
		$allow_frontend = apply_filters( 'pixproof_allow_frontend', true, $meta_box );
		...
		$show_filters = 'pixproof_Meta_Box_Show_Filters';
		foreach ( get_class_methods( $show_filters ) as $filter ) {
			add_filter( 'pixproof_show_on', array( $show_filters, $filter ), 10, 2 );
		}
	...
	public function add_metaboxes() {
		...
		foreach ( $this->_meta_box['pages'] as $page ) {
			if ( apply_filters( 'pixproof_show_on', true, $this->_meta_box ) )
	...
	public static function show_form( $meta_box, $object_id = 0, $object_type = '' ) {
		...
		// get box types
		$types = pixproof_Meta_Box_types::get();

		// Use nonce for verification
		echo "\n<!-- Begin CMB Fields -->\n";
		wp_nonce_field( self::nonce(), 'wp_meta_box_nonce', false, true );
		do_action( 'pixproof_before_table', $meta_box, $object_id, $object_type );
		echo '<table class="form-table pixproof_metabox">';
		...
		foreach ( $meta_box['fields'] as $field ) {
			...
			$field['default'] = apply_filters( 'pixproof_default_filter', $field['default'], $field, $object_id, $object_type );
			// 'pixproof_std_filter' deprectated, use 'pixproof_default_filter' instead
			$field['default'] = apply_filters( 'pixproof_std_filter', $field['default'], $field, $object_id, $object_type );
			...
			// (assuming no one would want to save 'pixproof_no_override_val' as a value)
			$meta = apply_filters( 'pixproof_override_meta_value', 'pixproof_no_override_val', $object_id, $field, $object_type );

			// If no override, get our meta
			if ( $meta === 'pixproof_no_override_val' )
				$meta = self::get_data();
		...
		echo '</table>';
		do_action( 'pixproof_after_table', $meta_box, $object_id, $object_type );
		echo "\n<!-- End CMB Fields -->\n";
	...
	public static function save_fields( $meta_box, $object_id, $object_type = '' ) {
		...
		if ( ! apply_filters( 'pixproof_show_on', true, $meta_box ) )
			return;
		...
		do_action( "pixproof_save_{$object_type}_fields", $object_id, $meta_box['id'], $updated, $meta_box );
	...
	public static function sanitization_cb( $meta_value, $field = array() ) {
		...
		// Validation via 'pixproof_Meta_Box_Sanitize' (with fallback filter)
		return call_user_func( array( pixproof_Meta_Box_Sanitize::get(), $field['type'] ), $meta_value, $field );
	}
	 ...
	 * @uses apply_filters() Calls 'pixproof_override_option_get_$option_key' hook to allow
	...
	public static function _get_option( $option_key, $default = false ) {
		...
		$test_get = apply_filters( "pixproof_override_option_get_$option_key", 'pixproof_no_override_option_get', $default );

		if ( $test_get !== 'pixproof_no_override_option_get' )
			return $test_get;
	 ...
	 * @uses apply_filters() Calls 'pixproof_override_option_save_$option_key' hook to allow
	...
	public static function save_option( $option_key ) {
		...
		$test_save = apply_filters( "pixproof_override_option_save_$option_key", 'pixproof_no_override_option_save', $to_save );

		if ( $test_save !== 'pixproof_no_override_option_save' )
			return $test_save;
...
// Handle oembed Ajax
add_action( 'wp_ajax_pixproof_oembed_handler', array( 'pixproof_Meta_Box_ajax', 'oembed_handler' ) );
add_action( 'wp_ajax_nopriv_pixproof_oembed_handler', array( 'pixproof_Meta_Box_ajax', 'oembed_handler' ) );
 ...
 * @return array               Options array or specific field
 */
function pixproof_get_option( $option_key, $field_id = '' ) {
	return pixproof_Meta_Box::get_option( $option_key, $field_id );
}
 ...
 * @param int   $object_id  Object ID
 */
function pixproof_print_metaboxes( $meta_boxes, $object_id ) {
	foreach ( (array) $meta_boxes as $meta_box ) {
		pixproof_print_metabox( $meta_box, $object_id );
	}
}
 ...
 * @param int   $object_id Object ID
 */
function pixproof_print_metabox( $meta_box, $object_id ) {
	$cmb = new pixproof_Meta_Box( $meta_box );
	if ( $cmb ) {

		pixproof_Meta_Box::set_object_id( $object_id );
 ...
 * @param int   $object_id Object ID
 */
function pixproof_save_metabox_fields( $meta_box, $object_id ) {
	pixproof_Meta_Box::save_fields( $meta_box, $object_id );
}
 ...
 */
function pixproof_metabox_form( $meta_box, $object_id, $echo = true ) {

	$meta_box = pixproof_Meta_Box::set_mb_defaults( $meta_box );
	...
	pixproof_Meta_Box::set_object_type( pixproof_Meta_Box::set_mb_type( $meta_box ) );
	...
	if (
		...
		&& wp_verify_nonce( $_POST['wp_meta_box_nonce'], pixproof_Meta_Box::nonce() )
		&& $_POST['object_id'] == $object_id
	)
		pixproof_save_metabox_fields( $meta_box, $object_id );
