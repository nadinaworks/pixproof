...
class pixproof {
	...
	static function form($config, $processor) {
		$form = self::instance('PixproofForm', $config);
	...
	/**
	 * @param array configuration
	 * @return PixproofProcessor
	 */
	static function processor($config) {
		return self::instance('PixproofProcessor', $config);
	}
	 ...
	 * @throws Exception
	 */
	static function callback($key, PixproofMeta $meta) {
	...
	protected static $textdomain = 'pixproof_txtd';
